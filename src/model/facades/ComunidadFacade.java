...
/**
 * Clase ComunidadFacade para trabajar con comunidades
 * 
 * @author sisinf
 */
public class ComunidadFacade {

	private static final String CONSULTAR_TODAS_COMUNIDADES = "SELECT c.nombre, c.lat, c.long, c.id"
			+ " FROM bichico.comunidad c ";

	private static final String CONSULTA_COMUNIDAD = "SELECT c.id, c.nombre, c.lat, c.long"
			+ " FROM bichico.comunidad c " + "WHERE c.id=?";

	private static final String CONSULTA_REGIONES = "SELECT r.id, r.nombre, r.lat, r.long " + "FROM bichico.region r "
			+ "WHERE r.id_com=?";

	private static final String CONSULTA_FACTOR_REGIONES = "SELECT r.nombre, r.habitantes, r.lat, r.long, r.id id, f.valor "
			+ "FROM region r, factor f " + "WHERE " + "      r.id = f.id_region AND r.id_com = ? " + "  AND "
			+ "      f.fecha = (" + "            SELECT MAX(fecha) " + "            FROM factor f "
			+ "            WHERE f.id_region = r.id " + "          )";

	/**
	 * Devuelve un mapa de las regiones de una comunidad y su factor de riesgo
	 * asociado.
	 * 
	...
	public static Map<RegionVO, FactorVO> obtenerFactorRegiones(int idCom) {
		...
		try {
			...
			while (resultSet.next()) {
				regionVO = new RegionVO(resultSet.getString("nombre"), resultSet.getFloat("lat"),
						resultSet.getFloat("long"));
				regionVO.setHabitantes(resultSet.getInt("habitantes"));
				factorVO = new FactorVO((int) resultSet.getDouble("valor"), resultSet.getInt("id")
	...
	/**
	 * Lee las regiones de una determinada comunidad de la base de datos Y las
	 * asigna a su corresndiente comunidad
	 * 
	 * @param comunidadVO
	 * @param idCom
	 */
