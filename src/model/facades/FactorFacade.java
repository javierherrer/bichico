...
/**
 * Clase FactorFacade para trabajar con factor
 * 
 * @author sisinf
 */
public class FactorFacade {
	...
	private final static String INSERTAR_FACTOR = "INSERT INTO Bichico.factor (fecha, id_region, valor) "
			+ "VALUES (?,?,?)";

	private static final String CONSULTA_FACTOR = "SELECT * FROM Bichico.factor f WHERE f.id_region=?";
	...
	/**
	 * obtener factor de una region
	 * @param regionVO
	 * @return FactorVO de la region introducida por parametro
	 */
	public static FactorVO obtenerFactor(RegionVO regionVO) {
		...
		try {
			...
			if (resultSet.next()) {
				factorVO = new FactorVO(resultSet.getDate(FECHA), (int) resultSet.getFloat(VALOR),
